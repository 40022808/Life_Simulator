import { 过场动画开始, 弹窗函数, 游戏开始函数, 默认性别函数, 设置保存函数, 默认难度函数, 词条读取函数, 人物属性, 词条描述读取函数, 打开设置界面函数, 游戏内数据, 游戏内数据更新函数, 回合结束函数 } from "./函数库.js";
...
const 游戏界面_按钮_回合_结束_结束按钮 = document.getElementById('游戏界面_按钮_回合_结束_结束按钮');
游戏界面_按钮_回合_结束_结束按钮.addEventListener('click', () => {
    
    if (游戏界面_按钮_回合_结束_结束按钮.disabled) return; // 如果已禁用，则直接返回，不执行

    游戏界面_按钮_回合_结束_结束按钮.disabled = true; // 禁用按钮
    setTimeout(() => {
        游戏界面_按钮_回合_结束_结束按钮.disabled = false; // x秒后恢复可点击状态
    }, 2000);

    if (游戏内数据.是否可以结束回合 == "是") {
        回合结束函数();
    }
    else if (游戏内数据.是否可以结束回合 == "否") {
        弹窗函数("还不能结束回合!");
    }
